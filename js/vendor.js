...
function loadVendorDashboard(vendorId) {
    ...
    // Chart 1: Sales Analytics (Line Chart)
    ...
    if (salesCtx) {
        new Chart(salesCtx, {
            ...
            data: {
                ...
                datasets: [{
                    label: "Revenue (PKR)",
                    data: [120000, 185000, 240000, 190000, 310000, totalRevenue > 0 ? totalRevenue : 220000],
                    borderColor: "#2563eb",
    ...
    const prodCtx = document.getElementById("vendorProductChart");
    if (prodCtx) {
        const approvedCount = vendorProds.filter(p => p.status === "approved").length;
        const pendingCount = vendorProds.filter(p => p.status === "pending").length;
        const rejectedCount = vendorProds.filter(p => p.status === "rejected").length;
...
function loadVendorReports(vendorId) {
    ...
    // Render reports chart (Bar Chart)
    const ctx = document.getElementById("vendorReportsChart");
    if (ctx) {
        new Chart(ctx, {
            type: "bar",
            data: {
                ...
                datasets: [{
                    ...
                    data: [150000, 220000, 310000, 180000, 420000, totalRevenue > 0 ? totalRevenue : 300000],
